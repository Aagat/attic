...
func (p *Pandoc) ConvertToPDF(content []byte, from string, metadata map[string]string) ([]byte, error) {
	...
	args := []string{
		...
		"--variable", fmt.Sprintf("geometry:margin=%s,includeheadfoot,heightrounded,hmarginratio=1:1,width=\\paperwidth,textwidth=\\dimexpr\\paperwidth-2\\marginparwidth\\relax", profile.MarginSize),
		...
		"--variable", "header-includes=\\usepackage[export]{adjustbox}\\usepackage{graphicx}\\makeatletter\\let\\oldincludegraphics\\includegraphics\\renewcommand{\\includegraphics}[2][]{\\oldincludegraphics[width=\\textwidth,keepaspectratio,center]{#2}}\\makeatother\\setlength{\\parindent}{0pt}\\usepackage{float}\\floatplacement{figure}{H}\\usepackage[document]{ragged2e}",
